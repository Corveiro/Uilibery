...
-- Theme and helpers (visual improvements, non-invasive)
Library.Theme = {
...
function Library:TweenInstance(Instance, Time, OldValue, NewValue)
	-- central tween helper with a smoother easing
	local tz = TweenService:Create(Instance, TweenInfo.new(Time, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { [OldValue] = NewValue })
	tz:Play()
	return tz
end

-- Generic hover helper for UI elements. Non-invasive: only animates visuals.
function Library:MakeHover(Element, opts)
	opts = opts or {}
	local iconIn = opts.iconIn or 0
	local iconOut = opts.iconOut or 0.5
	local textIn = opts.textIn or 0
	local textOut = opts.textOut or 0.3
	local bgIn = opts.bgIn
	local bgOut = opts.bgOut

	local function onEnter()
		if Element:FindFirstChild("Icon") and Element.Icon:IsA("ImageLabel") then
			self:TweenInstance(Element.Icon, 0.16, "ImageTransparency", iconIn)
		end
		if Element:FindFirstChild("NameTab") then
			self:TweenInstance(Element.NameTab, 0.16, "TextTransparency", textIn)
		end
		if Element:FindFirstChild("Title") then
			-- Title inside many controls
			self:TweenInstance(Element.Title, 0.16, "TextTransparency", textIn)
		end
		if bgIn ~= nil then
			self:TweenInstance(Element, 0.16, "BackgroundTransparency", bgIn)
		end
	end

	local function onLeave()
		if Element:FindFirstChild("Icon") and Element.Icon:IsA("ImageLabel") then
			self:TweenInstance(Element.Icon, 0.16, "ImageTransparency", iconOut)
		end
		if Element:FindFirstChild("NameTab") then
			self:TweenInstance(Element.NameTab, 0.16, "TextTransparency", textOut)
		end
		if Element:FindFirstChild("Title") then
			self:TweenInstance(Element.Title, 0.16, "TextTransparency", textOut)
		end
		if bgOut ~= nil then
			self:TweenInstance(Element, 0.16, "BackgroundTransparency", bgOut)
		end
	end

	-- Connect safely (not all UI elements have MouseEnter/Leave, but most do)
	if Element.MouseEnter and Element.MouseLeave then
		Element.MouseEnter:Connect(onEnter)
		Element.MouseLeave:Connect(onLeave)
	else
		-- Fallback: InputBegan/InputEnded on GUIObject
		if Element.InputBegan and Element.InputEnded then
...
function Library:MakeDraggable(topbarobject, object)
	...
	local function UpdatePos(input)
		...
		local Tween = TweenService:Create(object, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Position = pos })
		Tween:Play()
	end
...
function Library:NewWindow(ConfigWindow)
	...
	TeddyUI_Premium.Parent = Players.LocalPlayer:WaitForChild("PlayerGui")
	...
	Line.Name = "Line"
	Line.Parent = Top
	Line.BackgroundColor3 = Library.Theme.Primary
	...
	LogoHub.Parent = Left
	LogoHub.BackgroundColor3 = Library.Theme.Primary
	...
	Desc.TextColor3 = Library.Theme.MutedText
	...
	Icon.BackgroundColor3 = Library.Theme.Primary
	...
	-- hover
	self:MakeHover(Minize, { iconIn = 0, iconOut = 0.5, textIn = 0, textOut = 0.3 })
	...
	Icon_2.BackgroundColor3 = Library.Theme.Primary
	...
	self:MakeHover(Large, { iconIn = 0, iconOut = 0.5 })
	...
	Icon_3.BackgroundColor3 = Library.Theme.Primary
	...
	self:MakeHover(Close, { iconIn = 0, iconOut = 0.5 })
	...
	Line_2.BackgroundColor3 = Library.Theme.Primary
	...
	IconSearch.BackgroundColor3 = Library.Theme.Primary
	...
	function Tab:T(t, iconid)
		...
		Choose_2.BackgroundColor3 = Library.Theme.Primary
		...
		-- hover for tab
		self:MakeHover(TabDisable, { textIn = 0, textOut = 0.3, iconIn = 0, iconOut = 0.5 })
		...
		function TabFunc:AddSection(RealNameSection)
			...
			UIGradient.Color = ColorSequence.new { ColorSequenceKeypoint.new(0.00, Color3.fromRGB(24, 24, 25)), ColorSequenceKeypoint.new(0.52, Library.Theme.Primary), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(24, 24, 25)) }
			...
			function SectionFunc:AddToggle(cftoggle)
				...
				Content.TextColor3 = Library.Theme.MutedText
				...
				function ToggleFunc:Set(Boolean)
					if Boolean then
						Library:TweenInstance(ToggleCheck, 0.28, "BackgroundColor3", Library.Theme.Primary)
						Library:TweenInstance(Check, 0.28, "Position", UDim2.new(0, 22, 0.5, 0))
						Library:TweenInstance(Check, 0.28, "BackgroundColor3", Library.Theme.Text)
					else
						Library:TweenInstance(ToggleCheck, 0.28, "BackgroundColor3", Color3.fromRGB(60, 60, 60))
						Library:TweenInstance(Check, 0.28, "BackgroundColor3", Color3.fromRGB(200, 200, 200))
						Library:TweenInstance(Check, 0.28, "Position", UDim2.new(0, 3, 0.5, 0))
					end
					self.Value = Boolean
				...
				self:MakeHover(Toggle, { textIn = 0, textOut = 0.3 })
				Toggle_Click.Activated:Connect(function()
					-- small ripple effect for feedback
					local ripple = Instance.new("Frame", Toggle)
					ripple.BackgroundColor3 = Library.Theme.Primary
					ripple.BackgroundTransparency = 0.7
					ripple.Size = UDim2.new(0, 6, 0, 6)
					ripple.AnchorPoint = Vector2.new(0.5, 0.5)
					ripple.Position = UDim2.new(1, -30, 0.5, 0)
					ripple.ZIndex = Toggle.ZIndex + 5
					ripple.Name = "ripple"
					local rcorner = Instance.new("UICorner", ripple)
					rcorner.CornerRadius = UDim.new(1, 0)
					local targetSize = UDim2.new(0, 240, 0, 120)
					self:TweenInstance(ripple, 0.38, "Size", targetSize)
					self:TweenInstance(ripple, 0.38, "BackgroundTransparency", 1)
			...
			function SectionFunc:AddButton(cfbutton)
				...
				Content_2.TextColor3 = Library.Theme.MutedText
				...
				ImageLabel.BackgroundColor3 = Library.Theme.Primary
				...
				-- hover + ripple
				self:MakeHover(Button, { textIn = 0, textOut = 0.3, iconIn = 0, iconOut = 0.5 })
				Button_Click.Activated:Connect(function()
					...
					rip.BackgroundColor3 = Library.Theme.Primary
					rip.BackgroundTransparency = 0.7
					rip.Size = UDim2.new(0, 8, 0, 8)
					rip.Position = UDim2.new(0.5, -4, 0.5, -4)
					local rc = Instance.new("UICorner", rip)
					rc.CornerRadius = UDim.new(1, 0)
					self:TweenInstance(rip, 0.36, "Size", UDim2.new(0, 260, 0, 120))
					self:TweenInstance(rip, 0.36, "BackgroundTransparency", 1)
					task.delay(0.38, function() if rip and rip.Parent then rip:Destroy() end end)
					cfbutton.Callback()
			...
			function SectionFunc:AddDropdown(cfdropdown)
				...
				Content_6.TextColor3 = Library.Theme.MutedText
				...
				Selects.BackgroundColor3 = Library.Theme.Surface
				...
				SearchFrame_2.BackgroundColor3 = Library.Theme.Surface
				...
				function DropFunc:Add(v)
					...
					UIGradient_3.Color = ColorSequence.new { ColorSequenceKeypoint.new(0.00, Color3.fromRGB(0, 0, 0)), ColorSequenceKeypoint.new(0.51, Library.Theme.Primary), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(0, 0, 0)) }
					...
					-- hover for option
					self:MakeHover(Option2, { textIn = 0, textOut = 0.5 })
			...
			function SectionFunc:AddInput(cftextbox)
				...
				Content_5.TextColor3 = Library.Theme.MutedText
				...
				TextboxFrame.BackgroundColor3 = Library.Theme.Surface
				...
				-- focus highlight (visual only)
				RealTextBox.FocusGained:Connect(function()
					Library:TweenInstance(TextboxFrame, 0.18, "BackgroundColor3", Color3.fromRGB(34, 34, 34))
				end)
				RealTextBox.FocusLost:Connect(function()
					Library:TweenInstance(TextboxFrame, 0.18, "BackgroundColor3", Library.Theme.Surface)
			...
			function SectionFunc:AddSlider(cfslider)
				...
				Content_3.TextColor3 = Library.Theme.MutedText
				...
				SliderFrame.BackgroundColor3 = Library.Theme.Surface
				...
				SliderDraggable.BackgroundColor3 = Library.Theme.Primary
				...
				function SliderFunc:Set(Value)
					...
					-- use helper tween for smooth knob animation
					local scale = (Value - cfslider.Min) / (cfslider.Max - cfslider.Min)
					self:TweenInstance(SliderDraggable, 0.28, "Size", UDim2.fromScale(scale, 1))
				end
			...
			function SectionFunc:AddParagraph(cfpara)
				...
				Content_4.TextColor3 = Library.Theme.MutedText
			...
			function SectionFunc:AddDiscord(DiscordTitle, InviteCode)
				...
				DiscordCard.BackgroundColor3 = Library.Theme.Background -- Cor ajustada para combinar com o tema escuro
				...
				JoinBtn.BackgroundColor3 = Library.Theme.DiscordBlue -- Azul Discord
	...
	-- small hover for floating button
	self:MakeHover(G2L["2"], { iconIn = 0, iconOut = 0.5 })
